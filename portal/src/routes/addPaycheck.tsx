...
const AddPaycheck = () => {
  ...
  const { data: payrollData, create, update } = useAPI<IPayroll>({
    url: "/payroll",
    id,
  ...
  const currentForm: IPayrollSetup = useMemo(() => {
    return {
      ...
      } as IPayrollSetup, Array.from(edited)),
      ..._.omit(payrollData, Array.from(edited)),
      ...form
    } as any
  }, [form, edited, payrollData, data])
  ...
  const onSubmit = async () => {
    const method = !!payrollData ? update : create
    await method({
      ...currentForm,
  ...
  return (
    <ContentContainer>
      <div className="">
        <Header history={`/se-medarbejder/${id}`} title="Opret lønseddel" />
      </div>
