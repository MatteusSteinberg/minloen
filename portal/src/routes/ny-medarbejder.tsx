            ...
            <div className="flex flex-col items-stretch justify-start gap-1">
                {fields.map((v) => (
                    <React.Fragment key={v.name}>
                        {v.type === "dropdown" && <Dropdown name={v.name} options={v.options || []} placeholder={v.placeholder} {...inputHandler(v.keyPath)} onChange={(v) => onChange?.(v.keyPath, v)} />}
                        {v.type === "boolean" && (
                            <div className="flex flex-row gap-2">
                                <span className="text-white">{v.placeholder}</span>
                                <input name={v.name} placeholder={v.placeholder} {...inputHandler(v.keyPath)} onChange={(ev) => onChange(v.keyPath, ev.target.checked)} type="checkbox" />
                            </div>
                        )}
                        {!["dropdown", "boolean"].includes(v.type as any) && <Input type="text" {...v} {...inputHandler(v.keyPath)} />}
                    </React.Fragment>
                ))}
            </div>
            {children}
