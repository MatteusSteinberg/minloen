...
import React, { HTMLInputAutoCompleteAttribute, useRef, useState } from "react"
...
type field = { placeholder: string; autocomplete?: HTMLInputAutoCompleteAttribute; keyPath: string; name: string; type?: React.HTMLInputTypeAttribute | "boolean" | "dropdown"; options?: Array<DropdownOption> }
...
const Section = ({ title, undertitle, error, form, onChange, fields, children }: CoworkerSection) => {
    const inputHandler = (field: field) => ({
        onChange: (ev: React.ChangeEvent<HTMLInputElement>) => {
            console.log(field.keyPath, ev.target.value)
            ...
            <div className="flex flex-col items-stretch justify-start gap-1">
                {fields.map((v) => (
                    <React.Fragment key={v.name}>
                        ...
                        {!["dropdown", "boolean"].includes(v.type as any) && <Input type={v.type || "text"} autocomplete={v.autocomplete} name={v.name} placeholder={v.placeholder} {...inputHandler(v)} />}
                    </React.Fragment>
                ))}
            </div>
            {children}
                        ...
                        <>
                            ...
                            <Section
                                fields={[
                                    ...
                                    ...(form.salaryType === "hourly" ? [{ keyPath: "hourlyWage", name: "hourlyWage", placeholder: "Timeløn" }] : []),
                            ...
                            <Section
                                fields={[
                                    ...
                                    { keyPath: "pension.companyContributionPercentage", autocomplete: "new-password", name: "pensionContribution", placeholder: "Firma bidrag %" },
                                    { keyPath: "pension.companyAmount", autocomplete: "new-password", name: "pensionAmount", placeholder: "Firma beløb" },
                                ]}
                                form={form}
                                error={error}
                            ...
                            <Section
                                fields={[
                                    ...
                                    { keyPath: "workplacePension.companyContributionPercentage", autocomplete: "new-password", name: "workplacePensionCompanyContribution", placeholder: "Firma bidrag %" },
                                    { keyPath: "workplacePension.companyAmount", autocomplete: "new-password", name: "workplacePensionCompanyAmount", placeholder: "Firma beløb" },
                                ]}
                                form={form}
                                error={error}
