import { BuildingOfficeIcon, LockClosedIcon, MapPinIcon } from "@heroicons/react/24/outline"
import { useCallback, useEffect, useRef, useState } from "react"
import Dropdown, { DropdownOption } from "../elements/Dropdown"
import Portal from "../globals/Portal"
import ChangeOrganization from "../layouts/userSettings/ChangeOrganization"
            ...
            <div className="flex flex-col sm:flex-row items-start justify-between gap-4 md:gap-[72px] h-full">
              ...
              <div className="flex-col gap-2 md:max-w-[220px] md:w-full hidden sm:flex">
                <button onClick={() => setSelectedComponent("profile")} className={`md:max-w-[200px] md:w-full w-[51px] h-[51px] flex flex-row items-center justify-center md:justify-start gap-2 md:px-6 md:py-3 border-2 border-solid rounded-2xl md:rounded-full ${selectedComponent === "profile" ? activeClass : inActiveClass}`}>
                  ...
                  <span className="hidden -mt-1 md:block">Profil</span>
                </button>
                ...
                <button onClick={() => setSelectedComponent("organization")} className={`md:max-w-[200px] md:w-full w-[51px] h-[51px] flex flex-row items-center justify-center md:justify-start gap-2 md:px-6 md:py-3 border-2 border-solid rounded-2xl md:rounded-full ${selectedComponent === "organization" ? activeClass : inActiveClass}}`}>
                  <BuildingOfficeIcon className={`w-6 h-6 ${selectedComponent === "organization" ? "text-lightPrimary dark:text-darkPrimaryLight" : "text-lightBorder dark:text-darkBorder"}`} />
                  <span className="hidden -mt-1 md:block">Virksomhed</span>
                </button>
              </div>
              <div className="flex flex-col items-start justify-between h-full gap-[42px] w-full">
                ...
                {selectedComponent === "organization" && <ChangeOrganization handleClose={handleClose} />}
              </div>
            </div>
          </div>
        </div>
