    ...
    return (
        <div className="w-full p-8 bg-white border border-solid dark:bg-darkPrimarySupport rounded-3xl shadow-custom border-lightBorder dark:border-darkBorder">
            <div className="flex flex-col gap-8">
                <p className="text-text dark:text-white font-medium-normal">Løn oversigt</p>
                <ul className="flex flex-col justify-start gap-2">
                    <ListItem name="Løn" value={salaryNumbers.salary} />
                    {salaryNumbers.supplement.map((supp, index) => {
                        return <ListItem key={supp.id} name={supp.name || "Tillæg "} value={supp.value} />
                    })}
                ...
                <div className="text-left text-text dark:text-white">
                    <p className="font-small-normal">Til udbetaling</p>
                    <p className="font-h3">{(salaryNumbers.newSalary || 0).toFixed(2)} DKK</p>
                </div>
                <Button background="primaryLight" onClick={onSubmit} color="text">
                    Gem lønseddel
                </Button>
            </div>
        </div>
