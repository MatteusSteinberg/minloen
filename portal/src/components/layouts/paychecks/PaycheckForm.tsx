...
function SalaryListSupplementOrDeduction({ list, type, formChange, add, remove }: DeductionOrSupplement) {
  ...
  return <div className="p-8 bg-white dark:bg-darkPrimarySupport rounded-3xl shadow-custom">
    <div className="mb-4">
    ...
    <div className="flex flex-col items-stretch justify-start gap-1">
      {list.map((item, index) => (
        <div className="flex flex-row items-stretch gap-1" key={item.id}>
          <Input
            onChange={ev => formChange?.(`${type}.${index}.name`, ev.target.value)}
            name={type + item.id} value={item.name} placeholder={`${name}-navn ${index + 1}`} type={"text"} />
          <Input
            onChange={ev => formChange?.(`${type}.${index}.value`, parseFloat(ev.target.value))}
            name={type + item.id} value={item.value} placeholder={`${name} mængde`} type={"number"} />
          <Button background="secondarySupport" color="text" onClick={() => remove?.(item.id)}>
            <StopCircleIcon className="block w-5 h-5 text-text dark:text-white" />
          </Button>
        </div>
      ))}
    </div>
...
const PaycheckForm = ({ payrollSetup, user, onFormChange }: IPayrollForm) => {
  ...
  return (
    <>
      ...
      <SalaryListSupplementOrDeduction
        list={payrollSetup?.supplements || []}
        type="supplements"
        formChange={onFormChange}
        add={() => addSalaryChange("supplements")}
        remove={(id) => removeSalaryChange("supplements", id)}
      />
      <SalaryListSupplementOrDeduction
        list={payrollSetup?.deduction || []}
        type="deduction"
        formChange={onFormChange}
        add={() => addSalaryChange("deduction")}
        remove={(id) => removeSalaryChange("deduction", id)}
      />
    </>
  )
}
