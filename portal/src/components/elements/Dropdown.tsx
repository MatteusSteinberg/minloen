...
const Dropdown = ({ options, name, placeholder, error, label, onChange, spacing, value }: Props) => {
  ...
  return (
    <div className={`relative block w-full ${spacing ? "mb-4" : "mb-0"}`}>
      ...
      {isVisible && (
        <div className={`${dropdownDirection} max-h-80 overflow-y-auto z-40 left-0 p-2 absolute rounded-[14px] text-text dark:text-white w-full h-auto inline-flex flex-col gap-2 items-start gap justify-start border-[1px] border-solid bg-white dark:bg-[rgba(33,33,34,1)] border-lightBorder dark:border-[#33363E]`} ref={dropdownRef}>
          {options.map((option) => (
