...
const Button = ({ background, color, icon, onClick, children }: IButton) => {
  ...
  return (
    <button onClick={innerOnClick} disabled={loading ? true : false}>
      {background === "primary" && (
        <div className={`text-${color} min-w-[220px] min-h-[56px] rounded-2xl border-solid border-[1px] bg-primary border-primary py-[17px] px-16 flex justify-center align items-center gap-3`}>
          {loading ? (
            ...
            <>
              ...
              <span className="font-small-semibold">{children}</span>
            </>
          )}
        </div>
      )}
