        ...
        <>
            ...
            <div ref={node} className={`absolute transition-all duration-150 right-0 md:right-0 transform ${toggleMenu ? "scale-100 opacity-100 visible" : "scale-95 opacity-0 invisible"}`}>
                <div className="absolute right-4  top-4 md:top-full md:right-0 w-[286px] mt-10 p-4 bg-white dark:bg-darkPrimarySupport border border-solid border-lightBorder dark:border-[rgba(51,54,62,0.50)] rounded-2xl shadow-[0px_48px_64px_-16px_rgba(0,0,0,0.25)]">
                    ...
                    <div className="flex flex-col px-4 mt-3 bg-lightPrimary dark:bg-darkSecondarySupport rounded-2xl">
                        <button onClick={() => setIsModalOpen(true)} className="flex items-center w-full h-12 text-white transition-colors dark:text-white dark:text-dark group font-standard-medium hover:text-darkPrimaryLight">
                            <svg className="inline-block w-6 h-6 mr-4 fill-white dark:fill-[#6c7275] transition-colors group-hover:fill-darkPrimaryLight" width="24" height="24" viewBox="0 0 24 24">
                        ...
                        <button onClick={unauthenticate} className="flex items-center w-full h-12 text-white transition-colors dark:text-dark group font-standard-medium hover:text-darkPrimaryLight">
                            <svg className="inline-block w-6 h-6 mr-4 fill-white dark:fill-[#6c7275] transition-colors group-hover:fill-darkPrimaryLight" width="24" height="24" viewBox="0 0 24 24">
